...
import { Copy, Plus, Info, X as XIcon, User, Receipt, ArrowUpDown } from 'lucide-react';
...
interface IntegralBillingTableProps {
  ...
  setIntegralItems: React.Dispatch<React.SetStateAction<Record<string, { serviceId: string; quantity: number; price: number; name: string }[]>>>;
  setShowIntegralSelectModal: (show: boolean) => void;
  setShowIntegralServiceModal: (studentId: string | null) => void;
...
export const IntegralBillingTable: React.FC<IntegralBillingTableProps> = ({
  ...
  return (
    <div className="space-y-8 mt-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm group hover:border-brand-blue/20 transition-all">
        <div className="flex items-center gap-5">
          <div className="w-14 h-14 bg-brand-blue/5 rounded-2xl flex items-center justify-center text-brand-blue shadow-inner group-hover:scale-105 transition-transform">
            <Info size={24} />
          </div>
          <div className="space-y-1">
            <h4 className="text-[11px] font-black text-slate-900 uppercase tracking-[0.2em]">Cobrança Integral Seletiva</h4>
            <p className="text-[12px] text-slate-500 font-medium max-w-xl leading-relaxed">
              Utilize esta aba para faturar serviços avulsos ou mensais para alunos que não seguem o fluxo de consumo padrão.
            </p>
          </div>
        </div>
        ...
        <motion.button
          ...
          className="px-10 py-5 bg-brand-blue text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-brand-blue/20 flex items-center justify-center gap-3 transition-all"
        >
          <Plus size={18} />
          Selecionar Alunos
        </motion.button>
        ...
        <table className="w-full text-left border-separate border-spacing-y-2">
          <thead>
            <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
              <th className="pb-4 px-6 text-center w-8">
                <input
                  type="checkbox"
          ...
          <tbody>
            {visibleInvoices.map((inv, idx) => {
              ...
              return (
                ...
                >
                  <td className="py-4 px-6 text-center rounded-l-2xl border-y border-l border-transparent group-hover:border-slate-100 transition-colors">
                    <input
                      type="checkbox"
                      className="w-5 h-5 rounded-lg border-2 border-slate-300 accent-brand-blue cursor-pointer transition-all focus:ring-4 focus:ring-brand-blue/10"
                      checked={isSelected}
                  ...
                  <td className="py-4 px-6 border-y border-transparent group-hover:border-slate-100 transition-colors">
                  ...
                  <td className="py-4 px-4 text-center border-y border-transparent group-hover:border-slate-100 transition-colors">
                    <input
                      ...
                      className="w-28 text-center py-2.5 bg-slate-50 border-2 border-slate-100 rounded-xl text-[10px] font-black focus:outline-none focus:ring-4 focus:ring-brand-blue/10 focus:border-brand-blue transition-all text-slate-700 uppercase"
                    />
                  </td>
                  <td className="py-4 px-4 text-center border-y border-transparent group-hover:border-slate-100 transition-colors">
                    <input
                      ...
                      className="text-[10px] font-black text-slate-600 bg-slate-50 border-2 border-slate-100 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-4 focus:ring-brand-blue/10 focus:border-brand-blue transition-all uppercase"
                    />
                  </td>
                  <td className="py-4 px-4 text-center border-y border-transparent group-hover:border-slate-100 transition-colors">
                    <div className={cn(
                  ...
                  <td className="py-4 px-6 text-right rounded-r-2xl border-y border-r border-transparent group-hover:border-slate-100 transition-colors">
                      ...
                      <div className="hidden group-hover:block absolute top-0 right-full mr-4 w-72 p-5 bg-slate-900/95 backdrop-blur-xl text-white text-[10px] font-bold rounded-[2rem] shadow-2xl z-[150] border border-white/10 ring-1 ring-white/20 animate-in fade-in zoom-in duration-200">
                        ...
                        <div className="space-y-2.5">
                          <div className="flex justify-between items-center text-slate-300">
                            <span className="font-black uppercase tracking-widest text-[9px]">Bruto Integral:</span> 
