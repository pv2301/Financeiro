        ...
        <table className="w-full text-left border-separate border-spacing-y-2 px-4 mb-96">
          <thead className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm shadow-sm">
            <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
              ...
              <th className="py-4 px-4 text-center">Boleto / Título</th>
              <th className="py-4 px-4 text-center">Vencimento</th>
              <th className="py-4 px-6 text-right">Líquido</th>
            </tr>
          </thead>
          <tbody>
              ...
              >
                ...
                <td className="py-3 px-4 border-y border-transparent group-hover:border-slate-100 transition-colors">
                  <button 
                    ...
                    className="group/btn flex flex-wrap gap-1 max-w-[280px] p-2 rounded-xl border-2 border-dashed border-slate-100 hover:border-brand-blue hover:bg-brand-blue/5 transition-all text-left bg-slate-50/50 max-h-[300px] overflow-y-auto scrollbar-hide"
                  >
                    {items.length > 0 ? (
                      items.map((it, idx) => (
                        <span key={idx} className="text-[8px] font-black uppercase tracking-tighter bg-white text-slate-600 px-2 py-0.5 rounded-lg border border-slate-200 shadow-sm group-hover/btn:border-brand-blue/20">
                          {it.quantity}x {it.name}
                        </span>
                      ))
                    ) : (
                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 px-2">
