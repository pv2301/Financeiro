            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-4 space-y-6">
                 <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
                    ...
                    <form onSubmit={handlePreApprove} className="space-y-4">
                       ...
                       <select value={preApproveRole} onChange={e => setPreApproveRole(e.target.value as any)} className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 font-black text-[10px] uppercase tracking-widest outline-none cursor-pointer">
              ...
              <div className="lg:col-span-8 bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                 <div className="p-4 border-b border-slate-50 bg-slate-50/50">
                    <h3 className="font-black text-slate-900 uppercase tracking-tight text-sm">Usuários Ativos</h3>
                 </div>
                 <div className="divide-y divide-slate-50">
                    {profiles.map(p => (
                      <div key={p.uid} className="p-4 flex items-center justify-between group hover:bg-slate-50 transition-all">
                         ...
                         <div className="flex items-center gap-4">
                            <select value={p.role} onChange={(e) => handleUpdateRole(p.uid, e.target.value as any)} disabled={!isSuperAdmin || p.uid === currentUserProfile?.uid} className="bg-slate-50 border border-slate-100 rounded-lg px-2 py-1 text-[9px] font-black uppercase tracking-widest outline-none disabled:opacity-50">
                               <option value="EDITOR">EDITOR</option>
